...
public class GUI {
		...
		p = new JPanel(){
			...
			public void paintComponent(Graphics g){
				...
				for (int x = 0; x < enemies.size(); x++){
					//System.out.println("There is enemy at " + enemies.get(x).getX()*xBuffer + "," + enemies.get(x).getY()*yBuffer);
					g.fillRect(enemies.get(x).getX()*xBuffer/*-(3*xBuffer)*/, enemies.get(x).getY()*yBuffer/*-(3*yBuffer)*/, xBuffer, yBuffer);	
				}
				//Enemy Above
	...
	public void updateEnemies(){
		...
		gui_kList.updateEnemies(enemies);
		//pls
	}

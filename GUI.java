			...
			public void paintComponent(Graphics g){
				...
				if (colliders.get(0).size() > 0){
					for (int x = 0; x < colliders.get(0).size(); x++){
						//System.out.println("Does " + Math.abs((colliders.get(0).get(x).getX()-(play.getX()*xBuffer))/xBuffer) + "<=5 AND " + Math.abs((colliders.get(0).get(x).getY()-(play.getY()*yBuffer))/yBuffer) + "<=5?");
						//if (Math.abs((colliders.get(0).get(x).getX()-(play.getX()*xBuffer))/xBuffer) <= 5 && Math.abs((colliders.get(0).get(x).getY()-(play.getY()*yBuffer))/yBuffer) <= 5){
